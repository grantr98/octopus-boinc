...
def cur_price(data, time):
    ...
    for a in data:
        timestamp = datetime.datetime.fromisoformat(a["valid_from"])
        if timestamp == time:
            logging.info(
                "Located: {valid_from} at price {price}".format(
                    valid_from=timestamp, price=a["value_inc_vat"]
                )
            )
            result = a
            break
...
def boinc(price, runtime):
    ...
    if price <= threshold:
        logging.info(
            "Price {price} less than threshold {th}".format(
...
def main():
    ...
    logging.info("Rounded time " + str(rd))

    a = fetch()["results"]

    price_now = cur_price(data=a, time=rd)
